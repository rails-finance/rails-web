// The Compound V3 listing's identity search, on its own so both halves of the listing can read
...
// onto /api/compound/positions's `wallet` param. Same shape as the pilots
// (lib/liquity-v2/search.ts, lib/polaris/search.ts).

/** What the search box can name here: a holder address (0x…40). A Compound V3 row is a (wallet,
 *  comet) account with no id of its own, and the box resolves no ENS name, so anything else
 *  names no identity. */
export function parseCompoundSearch(q: string): { ownerAddress?: string } {
  const v = q.trim();
...
export function namesIdentity(q: string | undefined): boolean {
  return Boolean(parseCompoundSearch(q ?? "").ownerAddress);
}
