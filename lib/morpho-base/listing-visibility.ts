// Visibility resolution for the Morpho Base listing.
//
...
//
// Status is a MULTI-SELECT over open / closed / liquidated — the route's own derivation —
// serialized as one comma-separated `status` URL param.
//
...
import { namesIdentity } from "@/lib/morpho-base/search";

/** The lifecycle buckets the Status facet exposes. */
export type MorphoBaseStatusBucket = "open" | "closed" | "liquidated";

/** Canonical order — used for stable URL/chip serialization and set compares. */
export const ALL_MORPHO_BASE_STATUS_BUCKETS: MorphoBaseStatusBucket[] = ["open", "closed", "liquidated"];

export function isMorphoBaseStatusBucket(v: string): v is MorphoBaseStatusBucket {
  return (ALL_MORPHO_BASE_STATUS_BUCKETS as string[]).includes(v);
}

/** Reduce an arbitrary token list to valid buckets, deduped, in canonical order. */
export function canonicalStatuses(values: string[]): MorphoBaseStatusBucket[] {
  const set = new Set(values.filter(isMorphoBaseStatusBucket));
  return ALL_MORPHO_BASE_STATUS_BUCKETS.filter((b) => set.has(b));
}

/** Order-independent equality of two bucket selections. */
export function sameStatusSet(a: MorphoBaseStatusBucket[], b: MorphoBaseStatusBucket[]): boolean {
 ...
 *  MorphoBaseListFilters satisfies it without an import cycle. */
export interface MorphoBaseVisibilityInput {
  status?: string[];
  q?: string;
}
 ...
 *  status once it does. */
export function defaultStatuses(f: MorphoBaseVisibilityInput): MorphoBaseStatusBucket[] {
  return namesIdentity(f.q) ? [...ALL_MORPHO_BASE_STATUS_BUCKETS] : ["open"];
}
...
export function effectiveStatuses(f: MorphoBaseVisibilityInput): MorphoBaseStatusBucket[] {
...
export function isAllStatuses(f: MorphoBaseVisibilityInput): boolean {
  return effectiveStatuses(f).length === ALL_MORPHO_BASE_STATUS_BUCKETS.length;
}
