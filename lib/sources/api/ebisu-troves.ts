// Ebisu troves listing — the `api` arm's presentation transform.
// ----------------------------------------------------------------------------
// The rails-server route (/api/ebisu/troves) does the structural work — filter,
// sort, paginate over mv_ebisu_positions — and returns the page slice already
// shaped as the canonical TroveSummary (per-branch decimals applied server-side,
// USD/CR = 0). This builder narrows each backend row to the fields the Ebisu
// card consumes (branch symbol, scaled collateral + ebUSD debt, interest rate,
...
export type EbisuTroveStatus = "open" | "closed" | "liquidated";
/** Backend sort keys (all chain-direct). */
export type EbisuTroveSort = "debt" | "coll" | "interestRate" | "lastActivity";

export interface EbisuTroveSummary {
  /** Trove NFT id (uint256 string) — unique WITHIN its branch. */
  id: string;
  status: EbisuTroveStatus;
  /** Branch display symbol (weETH | sUSDe | WBTC | LBTC | stcUSD). */
  collateralType: string;
  /** Collateral the Trove currently holds (display units). */
  collateral: number;
  collateralRaw: string;
  /** ebUSD debt the Trove currently owes (display units). */
  debt: number;
  debtRaw: string;
  /** Highest recorded collateral / ebUSD debt over this Trove's life (display
...
export interface RawEbisuTroveRow {
  id: string;
  status: EbisuTroveStatus;
  collateralType: string;
  /** Ownership, flat on the backend row (ebisu route, mig 150). */
  owner: string | null;
  lastOwner: string | null;
  ownerEns: string | null;
...
export function buildEbisuTroveRows(raw: RawEbisuTroveRow[], prices?: LiquityForkPriceMap): EbisuTroveSummary[] {
  return raw.map((r) => {
