// Basedollar troves listing — the `api` arm's presentation transform.
// ----------------------------------------------------------------------------
// The rails-server route (/api/basedollar/troves) does the structural work — filter,
// sort, paginate over mv_basedollar_positions — and returns the page slice already
// shaped as the canonical TroveSummary (per-branch decimals applied server-side,
// USD/CR = 0). This builder narrows each backend row to the fields the Basedollar
// card consumes (branch symbol, scaled collateral + BD debt, interest rate,
...
export type BasedollarTroveStatus = "open" | "closed" | "liquidated";
/** Backend sort keys (all chain-direct). */
export type BasedollarTroveSort = "debt" | "coll" | "interestRate" | "lastActivity";

export interface BasedollarTroveSummary {
  /** Trove NFT id (uint256 string) — unique WITHIN its branch. */
  id: string;
  status: BasedollarTroveStatus;
  /** Branch display symbol (WETH | wstETH | rETH | wcbBTC | cbETH). */
  collateralType: string;
  /** Collateral the Trove currently holds (display units). */
  collateral: number;
  collateralRaw: string;
  /** BD debt the Trove currently owes (display units). */
  debt: number;
  debtRaw: string;
  /** Highest recorded collateral / BD debt over this Trove's life (display
...
export interface RawBasedollarTroveRow {
  id: string;
  status: BasedollarTroveStatus;
  collateralType: string;
  /** Ownership, flat on the backend row (basedollar route, mig 150). */
  owner: string | null;
  lastOwner: string | null;
  ownerEns: string | null;
...
export function buildBasedollarTroveRows(
  raw: RawBasedollarTroveRow[],
