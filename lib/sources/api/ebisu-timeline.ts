// Ebisu trove timeline — the `api` arm's presentation transform (chain-state tier).
// ----------------------------------------------------------------------------
// rails-server returns the raw mv_ebisu_events rows for one (branch, troveId) — the
// Trove's absolute before/after collateral + ebUSD debt, the on-chain actor/action,
// and the batched flag. This transform maps each to a BaseActivityEvent + EbisuContext
// the chain-state cards consume. Collateral scales by the branch's decimals (WBTC/LBTC
// = 8, rest = 18); ebUSD debt is always 1e18. After-values are directly emitted; the
// deltas are after − before (chain-derived).
//
// SERVER-ONLY — imported from the /api/ebisu/* route handlers.
...
import { resolveBranch, DEBT_SYMBOL, DEBT_ADDRESS, DEBT_DECIMALS, EBISU_BRANCHES } from "@/lib/ebisu/asset-catalog";
import {
  classifyTroveAdjust,
  forkAdjustLabel,
  forkRateChangeLabel,
...
export interface EbisuTimelineResult {
...
/** One row of mv_ebisu_events, exactly as the rails timeline route projects it.
...
export interface MvRow {
   ...
   *  no operation row matched the event, absent on pre-migration responses (mig 168).
   *  The debt legs are ebUSD (18); the collateral legs are in the BRANCH's own
   *  units, which are 8 on WBTC and LBTC. */
   ...
   *  off a redeemCollateral row, absent on pre-migration responses (mig 168).
   *  `redemption_fee_coll` is this Trove's own keep; the rest describe the whole
   *  act. Ebisu has never been redeemed against, so on today's data these are
   *  NULL on every row — the transform reads them anyway, because the first
   *  redemption should light the card up without a code change. */
...
const LABELS: Record<EbisuEventType, string> = {
...
// The debt-movement epsilon (≈ $0.01 for the 18-decimal ebUSD) is hoisted to the
 ...
 *  Sub-dust residuals are dropped too — an interest figure of 0.000003 ebUSD is
 *  noise, and stating it would put a number on the card no reader can use.
 *
 *  `collDecimals` is the BRANCH's, passed in per row: the collateral legs are
 *  8-decimal on WBTC and LBTC and 18 on the rest. */
...
/** The redemption facts → ctx shape. Undefined off a redemption row, which on
 *  Ebisu's data is every row so far. Both `feeKeptColl` and `collSent` are in
 *  the BRANCH's collateral units, so they take the branch's decimals — scaling
 *  a WBTC fee by 1e18 would report a real amount as dust. */
 ...
 *  than the archive-node re-read behind mig 113. The filler's figure stays the
 *  fallback, and on Ebisu it carries the whole load today — the branches have
 *  been redeemed against zero times and liquidated seven, so every priced row
 *  the timeline has ever rendered came from the filler. */
function forkPriceOf(
  ...
  if (r.redemption_branch_price != null) {
    // The Liquity-fork price scale is 1e(36 − collateral decimals), so the same
    // column means a different number on each branch: WBTC and LBTC divide by
    // 1e28, weETH / sUSDe / stcUSD by 1e18. Hence branch.decimals per row and
    // never a constant — a hardcoded 18 would render a WBTC price 10^10 too
    // small, which still reads as a price rather than failing.
...
/** Transform raw mv_ebisu_events rows → { collateralType, troveId, events, totalEvents }. */
export function buildEbisuTimeline(rows: MvRow[], collateralType: string, troveId: string): EbisuTimelineResult {
  const events: BaseActivityEvent[] = rows.map((r, idx) => {
    ...
    const kind = r.action as EbisuEventType;
    const branch = resolveBranch(r.collateral) ?? EBISU_BRANCHES.weeth;
    ...
    // Spine flows: branch collateral + ebUSD debt, signed by which way each moved.
    ...
    return {
      ...
      actionLabel,
      flows,
      context: { protocol: "ebisu", data: ctx },
    };
  });
