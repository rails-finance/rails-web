// Visibility resolution for the Dolomite listing.
//
...
// / liquidated, where 'liquidated' names a CLOSED account — serialized as one comma-separated
// `status` URL param.
//
...
import { namesIdentity } from "@/lib/dolomite/search";

/** The lifecycle buckets the Status facet exposes. */
export type DolomiteStatusBucket = "open" | "closed" | "liquidated";

/** Canonical order — used for stable URL/chip serialization and set compares. */
export const ALL_DOLOMITE_STATUS_BUCKETS: DolomiteStatusBucket[] = ["open", "closed", "liquidated"];

export function isDolomiteStatusBucket(v: string): v is DolomiteStatusBucket {
  return (ALL_DOLOMITE_STATUS_BUCKETS as string[]).includes(v);
}

/** Reduce an arbitrary token list to valid buckets, deduped, in canonical order. */
export function canonicalStatuses(values: string[]): DolomiteStatusBucket[] {
  const set = new Set(values.filter(isDolomiteStatusBucket));
  return ALL_DOLOMITE_STATUS_BUCKETS.filter((b) => set.has(b));
}

/** Order-independent equality of two bucket selections. */
export function sameStatusSet(a: DolomiteStatusBucket[], b: DolomiteStatusBucket[]): boolean {
  if (a.length !== b.length) return false;
  const bs = new Set(b);
  return a.every((x) => bs.has(x));
}

/** Just the filter fields visibility depends on — structural, so the page's DolomiteListFilters
 *  satisfies it without an import cycle. */
export interface DolomiteVisibilityInput {
  status?: string[];
  q?: string;
}
 ...
 *  status once it does. */
export function defaultStatuses(f: DolomiteVisibilityInput): DolomiteStatusBucket[] {
  return namesIdentity(f.q) ? [...ALL_DOLOMITE_STATUS_BUCKETS] : ["open"];
}
...
export function effectiveStatuses(f: DolomiteVisibilityInput): DolomiteStatusBucket[] {
...
export function isAllStatuses(f: DolomiteVisibilityInput): boolean {
  return effectiveStatuses(f).length === ALL_DOLOMITE_STATUS_BUCKETS.length;
}
