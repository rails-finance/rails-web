// The Dolomite listing's identity search, on its own so both halves of the listing can read it
...
// onto /api/dolomite/positions's `owner` param. Same shape as the pilots
// (lib/liquity-v2/search.ts, lib/polaris/search.ts).

/** What the search box can name here: a holder address (0x…40). Dolomite's search box takes an
 *  owner, not a position id, and the box resolves no ENS name, so anything else names no
 *  identity. */
export function parseDolomiteSearch(q: string): { ownerAddress?: string } {
  const v = q.trim();
...
export function namesIdentity(q: string | undefined): boolean {
  return Boolean(parseDolomiteSearch(q ?? "").ownerAddress);
}
