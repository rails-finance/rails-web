// Visibility resolution for the Fluid listing.
//
// The resting view is the MIDDLE PATH (Miles, 2026-09-10, the roster rule): the bare directory
// shows the positions still open; a search naming a holder or a single position shows every
// status. The argument for it is written once, on the pilot —
// lib/liquity-v2/listing-visibility.ts.
//
// Status is a MULTI-SELECT over the two lifecycle buckets mv_fluid_positions answers with —
// open / closed — serialized as one comma-separated `status` URL param.
//
...
import { namesIdentity } from "@/lib/fluid/search";

/** The lifecycle buckets the Status facet exposes. */
export type FluidStatusBucket = "open" | "closed";

/** Canonical order — used for stable URL/chip serialization and set compares. */
export const ALL_FLUID_STATUS_BUCKETS: FluidStatusBucket[] = ["open", "closed"];

export function isFluidStatusBucket(v: string): v is FluidStatusBucket {
  return (ALL_FLUID_STATUS_BUCKETS as string[]).includes(v);
}

/** Reduce an arbitrary token list to valid buckets, deduped, in canonical order. */
export function canonicalStatuses(values: string[]): FluidStatusBucket[] {
  const set = new Set(values.filter(isFluidStatusBucket));
  return ALL_FLUID_STATUS_BUCKETS.filter((b) => set.has(b));
}

/** Order-independent equality of two bucket selections. */
export function sameStatusSet(a: FluidStatusBucket[], b: FluidStatusBucket[]): boolean {
  if (a.length !== b.length) return false;
  const bs = new Set(b);
  return a.every((x) => bs.has(x));
}

/** Just the filter fields visibility depends on — structural, so the page's FluidListFilters
 *  satisfies it without an import cycle. */
export interface FluidVisibilityInput {
  status?: string[];
  q?: string;
}
 ...
 *  status once it does. */
export function defaultStatuses(f: FluidVisibilityInput): FluidStatusBucket[] {
  return namesIdentity(f.q) ? [...ALL_FLUID_STATUS_BUCKETS] : ["open"];
}
...
export function effectiveStatuses(f: FluidVisibilityInput): FluidStatusBucket[] {
...
export function isAllStatuses(f: FluidVisibilityInput): boolean {
  return effectiveStatuses(f).length === ALL_FLUID_STATUS_BUCKETS.length;
}
