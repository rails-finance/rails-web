// Ebisu economics — the single-Trove tower, valued at the branch's own oracle.
// ----------------------------------------------------------------------------
// An Ebisu Trove is single-collateral (its branch token) + single-debt (ebUSD).
...
// with), debt at ebUSD's $1 redemption face (the protocol's own mechanism).
...
// Ebisu charges a user-set annual interest rate, so the ebUSD figure is the
...
import { positionCollateralProv, positionDebtProv, lifetimeFlowProv } from "@/lib/ebisu/event-provenance";
 ...
 */
export function ebisuLifetimeWithOpening(
...
export function computeEbisuEconomics(
  view: EbisuTroveView,
  events?: BaseActivityEvent[],
  /** The merged whole-history flows on a windowed page (see
   *  ebisuLifetimeWithOpening). Present, it IS the lifetime layer and
  ...
  return {
    ...
    interestNote: valued
      ? "Ebisu is a Liquity V2 fork: each Trove carries its own annual interest rate. The ebUSD figure is the Trove's debt at its last change — interest built up since isn't counted here, and the position card shows the live total. Collateral is valued at the branch's own price and the debt at ebUSD's $1 redemption face, the protocol's own reckoning on both sides. Lifetime bars sum the Trove's own recorded deltas; the borrowed total counts every debt increase its events recorded — new draws, upfront fees, and interest applied when an operation touched the Trove."
      : "Ebisu is a Liquity V2 fork: each Trove carries its own annual interest rate. The ebUSD figure is the Trove's debt at its last change; interest built up since isn't counted here. The branch's price wasn't available on this load, so the tower shows amounts only — it never asserts a partial dollar total. Lifetime bars sum the Trove's own recorded deltas; the borrowed total counts every debt increase its events recorded — new draws, upfront fees, and interest applied when an operation touched the Trove.",
  };
}
