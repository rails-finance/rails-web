// Ebisu Trove → share-card model. The bridge between `loadEbisuTroveTail`
// (the same server tail the trove page itself awaits) and the shared card
// renderer — no second read, no second source of truth for what this trove
// currently states.
//
...
import { DEBT_SYMBOL, shortId } from "@/lib/ebisu/asset-catalog";
...
const STATUS_WORD: Record<EbisuTroveSummary["status"], string> = {
  open: "Open",
  closed: "Closed",
  liquidated: "Liquidated",
};

export function ebisuShareCardModel(
  tail: ForkTroveTail<EbisuTroveSummary>,
