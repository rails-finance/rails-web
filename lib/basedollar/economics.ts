// Basedollar economics — the single-Trove tower, valued at the branch's own oracle.
// ----------------------------------------------------------------------------
// An Basedollar Trove is single-collateral (its branch token) + single-debt (BD).
...
// with), debt at BD's $1 redemption face (the protocol's own mechanism).
...
// Basedollar charges a user-set annual interest rate, so the BD figure is the
...
import type { BasedollarTroveView } from "@/components/protocol/basedollar/basedollar-position-card";
import type { BaseActivityEvent } from "@/lib/shared/types/event-shape";
import { isBasedollarEvent } from "@/lib/shared/types/event-shape";
import { positionCollateralProv, positionDebtProv, lifetimeFlowProv } from "@/lib/basedollar/event-provenance";
...
interface TroveFlows {
  ...
  borrowed: number; // BD debt added (draws + upfront fees + applied interest)
  repaid: number; // BD repaid, voluntary
  debtLiquidated: number; // BD cleared by liquidation
  debtRedeemed: number; // BD repaid by redemptions
}
...
function replayLifetime(events: BaseActivityEvent[]): TroveFlows {
  ...
  for (const ev of events) {
    if (!isBasedollarEvent(ev)) continue;
 ...
 */
export function basedollarLifetimeWithOpening(
...
export function computeBasedollarEconomics(
  view: BasedollarTroveView,
  events?: BaseActivityEvent[],
  /** The merged whole-history flows on a windowed page (see
   *  basedollarLifetimeWithOpening). Present, it IS the lifetime layer and
  ...
  return {
    ...
    interestNote: valued
      ? "Basedollar is a Liquity V2 fork: each Trove carries its own annual interest rate. The BD figure is the Trove's debt at its last change — interest built up since isn't counted here, and the position card shows the live total. Collateral is valued at the branch's own price and the debt at BD's $1 redemption face, the protocol's own reckoning on both sides. Lifetime bars sum the Trove's own recorded deltas; the borrowed total counts every debt increase its events recorded — new draws, upfront fees, and interest applied when an operation touched the Trove."
      : "Basedollar is a Liquity V2 fork: each Trove carries its own annual interest rate. The BD figure is the Trove's debt at its last change; interest built up since isn't counted here. The branch's price wasn't available on this load, so the tower shows amounts only — it never asserts a partial dollar total. Lifetime bars sum the Trove's own recorded deltas; the borrowed total counts every debt increase its events recorded — new draws, upfront fees, and interest applied when an operation touched the Trove.",
  };
}
