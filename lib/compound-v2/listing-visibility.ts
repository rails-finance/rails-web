// Visibility resolution for the Compound V2 listing.
//
...
import { namesIdentity } from "@/lib/compound-v2/search";

/** The lifecycle buckets the Status facet exposes. */
export type CompoundV2StatusBucket = "open" | "closed" | "liquidated";

/** Canonical order — used for stable URL/chip serialization and set compares. */
export const ALL_COMPOUND_V2_STATUS_BUCKETS: CompoundV2StatusBucket[] = ["open", "closed", "liquidated"];

export function isCompoundV2StatusBucket(v: string): v is CompoundV2StatusBucket {
  return (ALL_COMPOUND_V2_STATUS_BUCKETS as string[]).includes(v);
}

/** Reduce an arbitrary token list to valid buckets, deduped, in canonical order. */
export function canonicalStatuses(values: string[]): CompoundV2StatusBucket[] {
  const set = new Set(values.filter(isCompoundV2StatusBucket));
  return ALL_COMPOUND_V2_STATUS_BUCKETS.filter((b) => set.has(b));
}

/** Order-independent equality of two bucket selections. */
export function sameStatusSet(a: CompoundV2StatusBucket[], b: CompoundV2StatusBucket[]): boolean {
 ...
 *  CompoundV2ListFilters satisfies it without an import cycle. */
export interface CompoundV2VisibilityInput {
  status?: string[];
  q?: string;
}
 ...
 *  status once it does. */
export function defaultStatuses(f: CompoundV2VisibilityInput): CompoundV2StatusBucket[] {
  return namesIdentity(f.q) ? [...ALL_COMPOUND_V2_STATUS_BUCKETS] : ["open"];
}
...
export function effectiveStatuses(f: CompoundV2VisibilityInput): CompoundV2StatusBucket[] {
...
export function isAllStatuses(f: CompoundV2VisibilityInput): boolean {
  return effectiveStatuses(f).length === ALL_COMPOUND_V2_STATUS_BUCKETS.length;
}
