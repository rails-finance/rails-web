// The PWN listing's identity search, on its own so both halves of the listing can read it
...
// onto the in-memory search. Same shape as the pilots (lib/liquity-v2/search.ts,
// lib/polaris/search.ts).

/** What the search box can name here: a loan number (all digits), or a holder address (0x…40).
 *  Anything else names no identity. */
export function parsePwnSearch(q: string): { loanId?: string; ownerAddress?: string } {
  const v = q.trim();
  if (!v) return {};
  if (/^\d+$/.test(v)) return { loanId: v };
...
export function namesIdentity(q: string | undefined): boolean {
  const { loanId, ownerAddress } = parsePwnSearch(q ?? "");
  return Boolean(loanId ?? ownerAddress);
}
