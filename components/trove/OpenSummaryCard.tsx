  ...
  const hoverContextItems = useMemo(() => {
    ...
    items.push(
      <span key="lifecycle" className="text-slate-600 dark:text-slate-500">
        This trove was opened on{" "}
        <HighlightableValue type="openedDate" state="after" value={trove.activity.createdAt}>
          {formatDate(trove.activity.createdAt)}
        </HighlightableValue>{" "}
        and has been active for{" "}
        <HighlightableValue type="activeDuration" state="after" value={activeDurationInSeconds}>
          ...
          <div className="flex items-center gap-2 text-xs">
            {!showViewButton && (
              <span className="text-slate-400">
                Opened{" "}
                <HighlightableValue type="openedDate" state="after" value={trove.activity.createdAt} variant="card">
                  {formatDate(trove.activity.createdAt)}
                </HighlightableValue>
              </span>
            )}
            <span className="text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 rounded-lg px-2">
              <HighlightableValue
                type="activeDuration"
                state="after"
                value={(new Date().getTime() - new Date(trove.activity.createdAt).getTime()) / 1000}
                variant="card"
              >
                {formatDuration(trove.activity.createdAt, new Date())}
              </HighlightableValue>
        ...
        <div className="grid grid-cols-1 pt-2 p-4 gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <div className="flex items-center gap-1 mb-1">
                {trove.batch.isMember && (
                  <span className="text-xs font-semibold px-1 py-0.5 bg-pink-300 text-white dark:bg-pink-900/50 dark:text-pink-400 rounded-xs">
                    DELEGATED
                  </span>
                )}
          ...
          {showViewButton ? (
            ...
            <div className="flex justify-between items-end">
              <TroveCardFooter trove={trove} showViewButton={showViewButton} />
