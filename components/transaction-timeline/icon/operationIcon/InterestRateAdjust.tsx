...
export function InterestRateAdjustIcon({ tx, isFirst = false, isLast = false, isExpanded = false, isDelegate = false }: InterestRateAdjustIconProps) {
  ...
  const iconContent = isDelegate 
    ? (isIncrease ? <PercentIncreaseIcon /> : <PercentDecreaseIcon />) // Delegate icons (duplicates for now)
    : (isIncrease ? <PercentIncreaseIcon /> : <PercentDecreaseIcon />); // Individual icons
