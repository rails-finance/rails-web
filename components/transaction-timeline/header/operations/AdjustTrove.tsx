...
export function AdjustTroveHeader({ tx }: { tx: TroveTransaction }) {
  ...
  if (collChangeFromOperation) {
    ...
    content.push(<AssetAction key="collateral" action={action} asset={tx.collateralType} amount={Math.abs(collChangeFromOperation)} />);
  }

  if (debtChangeFromOperation) {
    const action = debtChangeFromOperation > 0 ? "Borrow" : "Repay";
    content.push(<AssetAction key="debt" action={action} asset={tx.assetType} amount={Math.abs(debtChangeFromOperation)} />);
  }
