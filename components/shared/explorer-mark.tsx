...
// The block explorer's own mark, per chain.
// ----------------------------------------------------------------------------
// The Etherscan glyph was inlined in event-card-footer.tsx when Etherscan was
// the only explorer. It moved here when Base arrived so the footer could carry
// a per-chain mark; the Base square was then retired from the footer again
// (see ExplorerMark) and the file now holds the two marks for their two jobs:
// the chain (ChainMark) and the explorer link (ExplorerMark).
...
/** The mark on a block-explorer link. Etherscan's own glyph on every chain:
 *  Basescan is an Etherscan-family explorer and wears the same mark as its
 *  brand, so the glyph names the destination truthfully on Base too — and
 *  the Base square that used to stand in for it there said "Base", not
 *  "explorer" (Miles, 2026-09-02). `chainId` is kept so a future explorer
 *  outside the family can carry its own mark without a call-site change. */
export function ExplorerMark({ chainId, size = 16 }: { chainId: ChainId; size?: number }) {
  void chainId;
  return (
    <svg
      width={size}
