...
// Export control for the Compound V3 position detail page — a thin wrapper over
// the shared <ExportMenu> that supplies the Compound Markdown serializer,
// mirroring components/protocol/spark/spark-export-menu.tsx. The wrapper exists
// so the page can lazy-load the whole export path (dropdown UX + serializer +
...
import { compoundPositionsToMarkdown, type CompoundPositionsMarkdownArgs } from "@/lib/compound/position-to-markdown";

type Props = Omit<CompoundPositionsMarkdownArgs, "generatedAt"> & {
...
export function CompoundExportMenu({ csvFilename, scopeNote, fetchAllEvents, queued, ...markdownArgs }: Props) {
  return (
    <ExportMenu
      buildMarkdown={() => compoundPositionsToMarkdown({ ...markdownArgs, generatedAt: new Date() })}
