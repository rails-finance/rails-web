...
// Export control for the f(x) position detail page — a thin wrapper over the
// shared <ExportMenu> that supplies the f(x) Markdown serializer, mirroring
...
import { fxPositionToMarkdown, type FxPositionMarkdownArgs } from "@/lib/fx/position-to-markdown";

type Props = Omit<FxPositionMarkdownArgs, "generatedAt"> & {
...
export function FxExportMenu({ csvFilename, scopeNote, fetchAllEvents, ...markdownArgs }: Props) {
  return (
    <ExportMenu
      buildMarkdown={() => fxPositionToMarkdown({ ...markdownArgs, generatedAt: new Date() })}
