...
// Basedollar event detail (chain-state tier) — adapter onto the shared ChainTruthDetail
// grid. Shows the Trove's resulting branch collateral + BD debt after this event,
...
import type { BasedollarContext } from "@/lib/shared/types/event-shape";
...
import { DEBT_SYMBOL, resolveBranch } from "@/lib/basedollar/asset-catalog";
...
export interface BasedollarEventDetailProps {
  ctx: BasedollarContext;
  txHash?: string;
  blockNumber?: number;
}
...
export function BasedollarEventDetail({ ctx, txHash, blockNumber }: BasedollarEventDetailProps) {
  ...
  // ── The mig-166 figures (the receipts the prose echoes) ───────────────────
  //
  ...
  // branch-wide redemption this Trove was a slice of.
