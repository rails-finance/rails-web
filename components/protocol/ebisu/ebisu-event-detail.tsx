...
// Ebisu event detail (chain-state tier) — adapter onto the shared ChainTruthDetail
// grid. Shows the Trove's resulting branch collateral + ebUSD debt after this event,
...
export interface EbisuEventDetailProps {
  ctx: EbisuContext;
  txHash?: string;
  blockNumber?: number;
}
...
export function EbisuEventDetail({ ctx, txHash, blockNumber }: EbisuEventDetailProps) {
  const coords: EbisuCoords = { txHash, blockNumber, collateralType: ctx.collateralSymbol, isBatched: ctx.isBatched };
  ...
  // branch-wide redemption this Trove was a slice of. No Ebisu branch has been
  // redeemed against yet, so this block renders nothing today; it is written the
  // same as every other fork's so the first one needs no code.
