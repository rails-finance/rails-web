...
// Export control for the MakerDAO vault detail page — a thin wrapper over the
// shared <ExportMenu> that supplies the Maker Markdown serializer, mirroring
...
import { makerVaultToMarkdown, type MakerVaultMarkdownArgs } from "@/lib/makerdao/position-to-markdown";

type Props = Omit<MakerVaultMarkdownArgs, "generatedAt"> & {
  /** Output file name for the CSV download, including the `.csv` extension. */
  csvFilename: string;
  /** Loads the vault's whole history for the CSV, on a page that drew a
...
export function MakerdaoExportMenu({ csvFilename, scopeNote, fetchAllEvents, ...markdownArgs }: Props) {
  return (
    <ExportMenu
      buildMarkdown={() => makerVaultToMarkdown({ ...markdownArgs, generatedAt: new Date() })}
