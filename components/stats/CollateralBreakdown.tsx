...
export function CollateralBreakdown({
  ...
  const handleSearch = (value: string) => {
    ...
    if (value.trim()) {
      ...
      const newUrl = pathname !== "/troves" ? `/troves?${params.toString()}` : `${pathname}?${params.toString()}`;
          ...
          >
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="text-2xl font-semibold text-slate-600">${formatNumber(stats.totalCollateralUsd)}</div>
              </div>
              <div className="flex justify-between items-center">
                <div>
                  <div className="text-xs text-slate-500">Total Debt</div>
                  <div className="text-lg font-medium text-slate-700">${formatNumber(stats.totalDebt)}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-500">Active Troves</div>
                  <div className="text-lg font-medium text-slate-700">{stats.openTroveCount.toLocaleString()}</div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 flex justify-end">
              <span
                className="text-sm text-white group-hover:text-white font-medium inline-flex items-center gap-1 rounded-full transition-all w-fit py-1 group-hover:bg-blue-500 bg-slate-300 pl-3 pr-2"
        ...
        <div className="">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-900 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-white">${formatNumber(stats.totalCollateralUsd)}</div>
            ...
            <div className="bg-slate-900 rounded-lg p-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-2xl font-bold text-white">${formatNumber(stats.totalDebt)}</span>
            ...
            <div className="bg-slate-900 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-white">{stats.openTroveCount.toLocaleString()}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-750 border-b border-slate-700">
              <tr>
                ...
                <th className="text-right px-6 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider">CR</th>
                <th className="text-right px-6 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider">
                  Active Troves
                </th>
            ...
            <tbody>
              {sortedData.map(([collateralType, stats]) => {
                ...
                const tvlShare = totalStats.totalTVL > 0 ? (stats.totalCollateralUsd / totalStats.totalTVL) * 100 : 0;
                const collateralRatio = calculateCollateralRatio(stats.totalCollateralUsd, stats.totalDebt);
                const avgDebt = stats.openTroveCount > 0 ? stats.totalDebt / stats.openTroveCount : 0;
                const avgCollateral = stats.openTroveCount > 0 ? stats.totalCollateral / stats.openTroveCount : 0;
                const activeRate = stats.troveCount > 0 ? (stats.openTroveCount / stats.troveCount) * 100 : 0;
                    ...
                    <tr className="hover:bg-slate-750 transition-colors border-b border-slate-700">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <span className="text-white">${formatNumber(stats.totalDebt)}</span>
                        ...
                        <td colSpan={7} className="px-6 py-4">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <div className="text-white font-medium">${formatNumber(avgDebt)}</div>
                            </div>
                            <div>
                              <div className="text-slate-400 mb-1">Total Troves</div>
                              <div className="text-white font-medium">{stats.troveCount.toLocaleString()}</div>
                            </div>
                            <div>
                              <div className="text-slate-400 mb-1">Active Rate</div>
                              <div className="text-white font-medium">{activeRate.toFixed(1)}%</div>
                            </div>
                          </div>
                        </td>
                      </tr>
