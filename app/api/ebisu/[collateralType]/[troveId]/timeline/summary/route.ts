...
// The opening balance for an Ebisu Trove — everything BELOW the block that
// `/api/ebisu/:collateralType/:troveId/timeline?recent=N` opened its window
...
// one branch collateral against ebUSD for its whole life, so the summary
...
export async function GET(
  ...
  try {
    const url = `${RAILS_API_URL}/api/ebisu/${encodeURIComponent(collateralType)}/${encodeURIComponent(troveId)}/timeline/summary?cutoffBlock=${encodeURIComponent(cutoffBlock)}`;
    ...
    console.error("Error fetching ebisu timeline summary from backend:", error);
