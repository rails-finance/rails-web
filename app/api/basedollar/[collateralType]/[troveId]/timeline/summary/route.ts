...
// The opening balance for a Basedollar Trove — everything BELOW the block that
// `/api/basedollar/:collateralType/:troveId/timeline?recent=N` opened its window
...
// one branch collateral against BD for its whole life, so the summary
...
export async function GET(
  ...
  try {
    const url = `${RAILS_API_URL}/api/basedollar/${encodeURIComponent(collateralType)}/${encodeURIComponent(troveId)}/timeline/summary?cutoffBlock=${encodeURIComponent(cutoffBlock)}`;
    ...
    console.error("Error fetching basedollar timeline summary from backend:", error);
