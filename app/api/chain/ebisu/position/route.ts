...
import { EBISU_BRANCHES, DEBT_SYMBOL, resolveBranch } from "@/lib/ebisu/asset-catalog";

// Chain arm of the Ebisu trove read — the exact live-head detail for ONE
...
export async function GET(request: NextRequest) {
  ...
  if (!branch) return NextResponse.json({ error: "branch must name an Ebisu branch" }, { status: 400 });
  ...
  try {
    ...
    console.error("Error loading Ebisu trove from chain:", error);
