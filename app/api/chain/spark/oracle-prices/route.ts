...
// On-chain oracle USD for arbitrary SparkLend reserve addresses — IAaveOracle
...
// edge caching. Near-clone of app/api/chain/aave-v3/oracle-prices.
...
export async function GET(request: NextRequest) {
  ...
  try {
    const map = await resolveAaveOraclePrices([{ oracle: SPARK_ADDRESSES.ORACLE, assets }]);
    const prices: Record<string, number> = {};
    for (const a of assets) {
      const p = aaveOraclePriceOf(map, SPARK_ADDRESSES.ORACLE, a);
      if (p != null && p > 0) prices[a] = p;
    }
    return NextResponse.json({ prices });
  } catch (error) {
    console.error("Error reading SparkLend oracle prices:", error);
    return NextResponse.json({ prices: {} });
  }
}
