...
// A wallet's whole life on Aave V3 Base: from the INDEX when the index can vouch
...
// rails-server's /api/aave-v3-base/timeline hands a wallet's rows over decoded
...
export async function GET(request: NextRequest) {
  ...
  try {
    const indexed = await loadAaveV3EventsFromIndex(
      ...
      console.error("Aave V3 Base index read failed, sweeping instead:", e);
      return null;
    });
    ...
    // millions of logs to reach a shallower one. The response carries the
    // horizon either way and the page renders the verdict.
    ...
    console.error("Error reading Aave V3 Base history:", error);
