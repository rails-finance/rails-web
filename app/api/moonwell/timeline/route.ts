...
// api arm of a Moonwell wallet's timeline — the LIVE rails-server index.
// rails-server returns the raw replayed mv_moonwell_events rows; we run the
// presentation transform (buildMoonwellTimeline: fixed-catalog symbols + signs)
// → BaseActivityEvent[]. No USD / no HF — chain-direct values only. Node runtime.
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = `${RAILS_API_URL}/api/moonwell/timeline?wallet=${encodeURIComponent(wallet)}${recentQs}`;
    ...
    console.error("Error fetching moonwell timeline from backend:", error);
