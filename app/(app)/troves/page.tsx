...
export default function TrovesPage() {
  ...
  const updateUrl = useCallback(
    (updates: Record<string, string | number | string[] | undefined>, options?: { scroll?: boolean }) => {
      ...
      if (needsClientSidePagination) {
        ...
        if (filters.collateralTypes && filters.collateralTypes.length > 0) {
          for (const collType of filters.collateralTypes) {
            ...
            params.set(
              "status",
              filters.status === "active" ? "open" : filters.status === "closed" ? "closed" : "open",
            );
      ...
      <div className="max-w-7xl mx-auto py-8">
        ...
        {troves.length > 0 ? (
          <div className="grid grid-cols-1 gap-6">
            {troves.map((trove) => (
              <SimplifiedTroveCard key={trove.id} trove={trove} showViewButton hideLabels={false} />
            ))}
          </div>
        ) : (
