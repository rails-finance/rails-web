...
export default function AaveV3PositionNotFound() {
  return (
    <RouteNotFound
      session="aave-v3"
      heading="Not an account address"
      backHref="/ethereum/aave-v3"
      backLabel="Browse Aave V3 positions"
    >
      This page is about one account, named by its 20-byte address. The URL does not carry one, so there is no account
      to look up — an address Aave V3 has simply never seen would still resolve, and say so.
    </RouteNotFound>
  );
}
