...
export default function MaplePositionNotFound() {
  return (
    <RouteNotFound
      session="maple"
      heading="Not an account address"
      backHref="/ethereum/maple"
      backLabel="Browse Maple positions"
    >
      This page is about one account, named by its 20-byte address. The URL does not carry one, so there is no account
      to look up — an address Maple has simply never lent to would still resolve, and say so.
    </RouteNotFound>
  );
}
