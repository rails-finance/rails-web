...
export default function MoonwellPositionNotFound() {
  return (
    <RouteNotFound
      session="moonwell"
      heading="Not an account address"
      backHref="/ethereum/moonwell"
      backLabel="Browse Moonwell positions"
    >
      This page is about one account, named by its 20-byte address. The URL does not carry one, so there is no account
      to look up — an address Moonwell has simply never seen would still resolve, and say so.
    </RouteNotFound>
  );
}
