...
// Asymmetry Trove detail — reference depth, chain-state-first, the 3-section
...
// branch's own contracts via /api/chain/asymmetry/position — getLatestTroveData
...
import { AsymmetryEventCard } from "@/components/protocol/asymmetry/asymmetry-event-card";
import {
  AsymmetryPositionCard,
  viewFromSummary,
  type AsymmetryTroveView,
} from "@/components/protocol/asymmetry/asymmetry-position-card";
...
const FORK_RUNS = liquityForkTimelineRuns({ is: isAsymmetryEvent, debtSymbol: DEBT_SYMBOL });

interface AsymmetryTroveDetailProps {
  collateralType: string;
  troveId: string;
  /** The server read's tail. Null on an SSR miss — this component then fetches. */
  initialTrove: AsymmetryTroveSummary | null;
  initialEvents: BaseActivityEvent[] | null;
  initialCutoffBlock: number | null;
  initialOpening: TimelineOpeningBalance | null;
}

export default function AsymmetryTroveDetail({
  collateralType,
  troveId,
  initialTrove,
  initialEvents,
  ...
  const [view, setView] = useState<AsymmetryTroveView | null>(() =>
    ...
    (async () => {
      ...
      try {
        const [pData, tData] = await Promise.all([
          fetchAsymmetryTroves({ troveId, collateralTypes: [collateralType], limit: 1 }),
          fetchAsymmetryTimeline(collateralType, troveId, { recent: TIMELINE_WINDOW_EVENTS }),
        ]);
  ...
  useEffect(() => {
    ...
    fetchTimelineOpeningBalance({
      path: `/api/asymmetry/${encodeURIComponent(collateralType)}/${encodeURIComponent(troveId)}/timeline/summary`,
      params: {},
      cutoffBlock,
      signal: ac.signal,
    })
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchLiquityForkPosition({ protocol: "asymmetry", branch: collateralType, troveId });
  ...
  const asymmetryEvents = useMemo(() => events.filter(isAsymmetryEvent), [events]);
  const tl = useTimelineEvents(asymmetryEvents, {
  ...
  const lifetimeEvents = lifetimeKnown ? asymmetryEvents : undefined;
  const precomputedLifetime = useMemo(
    () => asymmetryLifetimeWithOpening(asymmetryEvents, opening),
    [asymmetryEvents, opening],
  );
  ...
  const fetchAllHistory = useCallback(async () => {
    const res = await fetchAsymmetryTimeline(collateralType, troveId);
    const served = (res.events ?? []).filter(isAsymmetryEvent);
  ...
  return (
    <div className="py-8 space-y-6">
      <DetailTopRow session="asymmetry" wallet={view?.owner ?? view?.lastOwner ?? null}>
        {view && (
          <LiquityForkExportMenu
            ...
            csvFilename={`asymmetry-${collateralType}-${troveId.slice(0, 10)}-activity.csv`}
            fetchAllEvents={historyWindow.state === "whole" ? undefined : fetchAllHistory}
            history={markdownHistoryScope(historyWindow, asymmetryEvents)}
            scopeNote={exportScopeNote(historyWindow, asymmetryEvents, "this Trove's whole history")}
          />
        )}
      </DetailTopRow>
      ...
      {loading ? (
        ...
        <>
          ...
          {view &&
            (() => {
              const towerData = computeAsymmetryEconomics(view, lifetimeEvents, precomputedLifetime);
              const forkOpts = {
                name: "Asymmetry",
                debtSymbol: DEBT_SYMBOL,
          ...
          <ChainTruthTimeline
            ...
            // Matches `AsymmetryEventCard`'s own
            // `persistKey={`asymmetry:${event.id}`}` — lets pinned mode (the
