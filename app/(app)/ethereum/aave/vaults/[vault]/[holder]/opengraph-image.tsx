// One vault position's share card on Ethereum — the census row, and nothing
...
export const alt = "This address's position in an Aave vault on Ethereum";

// The page's own gates, applied here BEFORE the census proxy is asked: the
// page gates the vault with the same address shape, and the HOLDER must be an
// address — a typed name is resolved by the page, never by an image route
// (metadata and share cards make no network call of their own), and the card
// matches the census row by address, so a non-address holder can draw nothing
// and must cost nothing (scripts/verify/verify-share-abuse.mjs holds the
// census).
...
export default async function Image({ params }: Props) {
  ...
  return positionImage({
    // No `session`: the Vaults section has no roster entry and no static card
    // of its own, so a failed render falls back to the home card.
    load: async () => {
      const typed = decodeURIComponent(holder);
      if (!ADDRESS.test(vault) || !ADDRESS.test(typed)) return null;
