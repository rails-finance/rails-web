...
export default function MoonwellBasePositionNotFound() {
  return (
    <RouteNotFound
      session="moonwell-base"
      heading="Not an account address"
      backHref="/base/moonwell"
      backLabel="Browse Moonwell positions on Base"
    >
      This page is about one account, named by its 20-byte address. The URL does not carry one, so there is no account
      to look up — an address the Comptroller has simply never seen would still resolve, and say so.
    </RouteNotFound>
  );
}
