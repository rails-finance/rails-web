// The route-level skeleton for Morpho's vault layer on Base.
// ----------------------------------------------------------------------------
...
// flushed the response status is fixed — so at `morpho/vaults/` this file would
// also wrap `[vault]` and `[vault]/[holder]`, and each of their `notFound()`
// calls would run too late to matter: a vault that does not exist would answer
// 200 with a not-found body. The group holds the roster, the position listing
// and the about page, all of which want this skeleton, and no dynamic segment;
// a route group adds no URL segment, so `/base/morpho/vaults` is still the
// roster's path and nothing moved for a visitor.
...
// pathname resolves to the Morpho Blue Base roster entry, so the real rail is
// drawn with the VAULTS tab lit. That is the same chrome the real page has, so
// the row holds steady through the navigation instead of vanishing into the
// skeleton.
